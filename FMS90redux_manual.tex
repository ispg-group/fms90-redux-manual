...
%2 INTRO
The AIMS module implements the Ab Initio Multiple Spawning method to perform dynamics 
calculations on multiple electronic states. 
...
\newpage \section{Building FMS \bfec{to be updated}}
\label{build}
...
\item (optional) Run \verb*$make test$ to run the battery of test jobs in the ``examples''directory. The tests will be automatically checked for any run-time errors; the results should be manually verified.
\end{enumerate}
...
After a CONFIGFMS file has been created, type \verb*$make$ to compile the FMS code. An executable will be created and placed in the bin/ directory. FMS utilities will also be compiled and placed in the util/ directory (except in the case of FMSMolpro).
...
This section provides an overview of the basic FMS program flow for AIMS dynamics. The reader is assumed to be familliar with the fundamental principles of AIMS; see the FMS bibliography (sec. \ref{sec:aimsbiblio}) for a list of references. For details of other calculation types, see section \ref{other}.


\subsection{Initial Conditions}
\label{initial} 

Prior to the beginning of AIMS dynamics, initial conditions may be generated from an adequate ground-state distribution. FMS provides a variety of sampling methods, depending on the system type. Alternatively, initial coordinates and momenta may be specified manually in Geometry.dat (sec. \ref{geomdat}).

Initial conditions are generated in the following manner (with corresponding namelist variables):

\paragraph{QM Calculations}
For purely QM PESs, initial conditions are sampled from the harmonic oscillator states of a PES minimum, using one of several semi-classical transformations (sec. \ref{initial}). The geometry of the PES minimum is read from Geometry.dat (sec. \ref{geomdat}), while the frequencies or hessian of the minimum are read from their corresponding files (sec. \ref{freqfile}).
...
Each FMS run must take place in its own directory.
...
\hline
 ...
 Hessian.dat OR Frequencies.dat & Frequencies input for initial condition sampling (sec. \ref{freqfile}) \\ \hline 
\end{tabular}
...
{\tt SpawnCoupV}     & {\tt Logical (.false.)}& {\small Use Coup.velocity as criterion for spawning}\\&&
                                                {\small rather than Coupling magnitude alone.}\\
...
%3 Geometry.dat
\subsection{Geometry.dat\label{aims:geometry}}
...
%
%
%\subsection{Mopac and Amber}
%This interface is selected with \verb*$model=`mopac'$ in Control.dat.
%
%Mopac is an ESP capable of generating semi-empirical PESs. It has also been interfaced with Amber, an MM package for generating QM/MM PESs. 
%\label{mpa} 
%
%
%
%
%
%\subsection{Molpro and Tinker}
%\label{mpt} 
%This interface is selected with \verb*$model=`molpro'$ in Control.dat.
%
%Molpro is a suite of \textit{ab initio} electronic structure codes;
%FMS can use any PES generated by molpro for single state BOMD, but can
%currently use only CAS and CASPT2 PESs for full multistate AIMS
%propagation. For QM/MM simulations with molpro, the Tinker package is
%used for the MM system. The current implementation does not allow for
%bonding across the QM/MM boundary.
% 
%\subsubsection{Additional Input Files}
%
%\begin{itemize}
% \item  \textit{name}.com : molpro input file. This file contains instructions for molpro's electronic structure calculations. It can be generated using the 'deckMP' script in the 'util' directory of the FMS distribution (sec. \ref{deckmp}).
% \item  \textit{fmsname}.in.xyz: tinker xyz file (for QM/MM runs only). The value of \textit{fmsname} is specified in Control.dat. This file contains the geometry and atom types for the entire system. Note that the QM system must be listed before the MM system, and in the same order as in Control.dat. The QM geometry will be overwritten unless InitialCond=0 in Control.dat. The MM geometry may be modified using the GenSolvent, RelaxSteps, and various equilibration keywords in Control.dat.
% \item \textit{fmsname}.key: tinker keyword file (for QM/MM runs
%only). The value of \textit{fmsname} is specified in Control.dat. This
%file contains various keywords to direct tinker's calculation of the
%MM force field. It MUST contain these 2 entries:
%
% \begin{minipage}{.7\linewidth} \begin{verbatim}
%parameters    [/path/to/parameters/paramfile.prm] 
%inactive      -1   [Num QM atoms] 
%\end{verbatim} \end{minipage} 
%\end{itemize}
%
...
FMS90 contains a series of (analytical) model systems that can be used for development or to get acquinted with a typical AIMS simulation without the associated electronic-structure costs. The following models are implemented:

\begin{itemize}
\item[\texttt{gliMethod=2}] \bfec{check}
\end{itemize}
...
%\paragraph{deckMP.e}
%\label{deckmp}
%Creates an input deck for use with FMSMolpro or FMSMolproTinker. Requires a Geometry.dat file for QM runs or a .in.xyz file for QM/MM runs.
%
%
%
%
...
%
%Removes any and all output files that may be recovered by running DumpText.e. Note that it does not remove FMS.out or any electronic-structure specific output. Run CleanText.e in the directory containing the text files to be removed.
...
%Reprints of Martinez group articles are available from 
%\url{http://mtzweb.stanford.edu/reprints0.html}
%
%\begin{itemize}
%
...
%\begin{itemize}
%
%\item Jay~William Ponder.
%\newblock Tinker, 2004. \url{http://dasher.wustl.edu/tinker/}
%
%\item H.-J. Werner, P.~J. Knowles, R.~Lindh, M.~Sch\"utz, P.~Celani, T.~Korona, F.~R.
%  Manby, G.~Rauhut, R.~D. Amos, A.~Bernhardsson, A.~Berning, D.~L. Cooper,
%  M.~J.~O. Deegan, A.~J. Dobbyn, F.~Eckert, C.~Hampel, G.~Hetzer, A.~W. Lloyd,
%  S.~J. McNicholas, W.~Meyer, M.~E. Mura, A.~Nicklass, P.~Palmieri, R.~Pitzer,
%  U.~Schumann, H.~Stoll, A.~J. Stone, R.~Tarroni, and T.~Thorsteinsson.
%\newblock Molpro, version, a package of ab initio programs, 2006. \url{http://www.molpro.net/}
%
%\item HDF5 (library for writing output) - \url{http://hdfgroup.org/}
%
